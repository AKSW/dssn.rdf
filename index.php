...
class Mimeparse {
     ...
     * @return array ($type, $subtype, $params)
     */
    public static function parse_media_range($range) {
      list ($type, $subtype, $params) = self::parse_mime_type($range);
     ...
     */
    public static function fitness_and_quality_parsed($mime_type, $parsed_ranges) {
      $best_fitness = -1;
      $best_fit_q   = 0;
      list ($target_type, $target_subtype, $target_params) = self::parse_media_range($mime_type);
     ...
     * @return float $q
     */
    public static function quality_parsed($mime_type, $parsed_ranges) {
      list ($fitness, $q) = self::fitness_and_quality_parsed($mime_type, $parsed_ranges);
      return $q;
    }
     ...
     * @return unknown
     */
    public static function quality($mime_type, $ranges) {
      $parsed_ranges = explode(',', $ranges);

      foreach ($parsed_ranges as $i=>$r)
          $parsed_ranges[ $i ] = self::parse_media_range($r);

      return self::quality_parsed($mime_type, $parsed_ranges);
    }
     ...
     */
    public static function best_match($supported, $header) {
      $parsed_header = explode(',', $header);

      foreach ($parsed_header as $i=>$r)
          $parsed_header[ $i ] = self::parse_media_range($r);

      $weighted_matches = array();
      foreach ($supported as $mime_type) {
          $weighted_matches[] = array(
            self::fitness_and_quality_parsed($mime_type, $parsed_header),
            $mime_type
          );
      }
